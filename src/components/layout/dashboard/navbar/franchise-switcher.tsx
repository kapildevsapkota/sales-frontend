...
export function FranchiseSwitcher({ className }: FranchiseSwitcherProps) {
  ...
  // Find the currently active franchise in our hardcoded list
    ...
    <div className={cn("flex items-center gap-2", className)}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="outline"
            className="flex h-10 items-center justify-between gap-2 px-3 border-gray-200 hover:bg-gray-50 bg-white -sm transition-all duration-200 min-w-[200px] max-w-[280px]"
        ...
        <DropdownMenuContent align="start" className="w-[240px] p-1.5 -lg border border-gray-100 rounded-xl">
          <div className="px-2 py-1.5 mb-1 border-b border-gray-50">
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">
              Switch Franchise
            </p>
          </div>
          {HARDCODED_FRANCHISES.map((franchise) => {
            const isActive = Number(franchise.id) === currentFranchiseId;
            ...
            return (
              <DropdownMenuItem
                ...
                disabled={isSwitching !== null}
                className={cn(
                  "flex items-center justify-between px-2.5 py-2 rounded-lg cursor-pointer transition-colors duration-150 focus:bg-gray-50 my-0.5",
                  isActive && "bg-blue-50/50 text-blue-700 focus:bg-blue-50/50"
                )}
              >
                <div className="flex items-center gap-2 truncate">
                  <Building2 className={cn("h-4 w-4 text-gray-400", isActive && "text-blue-500")} />
                  <span className={cn("text-xs text-gray-700 font-medium truncate", isActive && "text-blue-700 font-semibold")}>
                    {franchise.name}
                  </span>
                </div>
                {isActive && (
