...
export function DashLocationCell({ sale }: DashLocationCellProps) {
  ...
  const handleLocationSelect = async (location: DashLocation) => {
    try {
      ...
      toast({
        title: "Success",
        description: `Location for order #${sale.id} updated to ${location.name}.`,
      });
    } catch (error) {
  ...
  const handleSendOrder = async () => {
    try {
      ...
      toast({
        title: "Success",
        description: `Order #${sale.id} sent to dash.`,
      });
    } catch (error) {
      console.error("Error sending order to dash:", error);
      toast({
        title: "Error",
        description: "Failed to send order to dash.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button onClick={handleSendOrder} disabled={!sale.dash_location_name}>
        <SendIcon size={12} />
      </Button>
    </div>
  );
