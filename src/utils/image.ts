...
export const compressImage = async (
  ...
  try {
    ...
    console.log(`[Image Compressor] Original size: ${(file.size / 1024).toFixed(2)} KB`);
    const compressedFile = await imageCompression(file, compressionOptions);
    console.log(`[Image Compressor] Compressed size: ${(compressedFile.size / 1024).toFixed(2)} KB`);
